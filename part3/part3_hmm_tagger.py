...
for idx, sent in enumerate(test_sents):
    ...
    for i in range(len(words)):
        ...
        if g == p:
            ...
            errors.append({"word": w, "gold": g, "pred": p, "is_oov": is_oov})
...
print(
    f"Seen Words Accuracy: {correct_seen / total_seen * 100:.2f}% ({correct_seen}/{total_seen})"
)
print(
    f"OOV Words Accuracy: {correct_oov / total_oov * 100:.2f}% ({correct_oov}/{total_oov})"
)
print("\n" + "=" * 50 + "\n")
...
ambig_errors = [
    e for e in errors if not e["is_oov"] and len(word_tag_counts[e["word"]]) > 1
]
other_errors = [
    e for e in errors if not e["is_oov"] and len(word_tag_counts[e["word"]]) <= 1
]
...
print(
    f"Ambiguity errors: {len(ambig_errors)} ({len(ambig_errors) / len(errors) * 100:.1f}%)"
)
print(
    f"Other errors: {len(other_errors)} ({len(other_errors) / len(errors) * 100:.1f}%)"
)
print()
...
for e in oov_errors[:3]:
    selected.append(
        (
            ...
            e["gold"],
            e["pred"],
            "OOV word - emission is uniform, transition determines tag",
        )
    )
for e in ambig_errors[:4]:
    ...
    selected.append(
        (
            ...
            e["gold"],
            e["pred"],
            f"Lexical ambiguity - can be {', '.join(possible_tags)}",
        )
    )
for e in other_errors[:3]:
    selected.append(
        (
            ...
            e["gold"],
            e["pred"],
            "Sparse transition context or tag sequence mismatch",
        )
    )
